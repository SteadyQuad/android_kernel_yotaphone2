...
struct cyttsp5_core_driver {
	...
	struct dentry *(*request_tthe_debugfs_dir)(struct cyttsp5_device *ttsp);
...
#ifdef TTHE_TUNER_SUPPORT
static inline struct dentry *cyttsp5_request_tthe_debugfs_dir(
		struct cyttsp5_device *ttsp)
{
	struct cyttsp5_core *cd = ttsp->core;
	struct cyttsp5_core_driver *d = to_cyttsp5_core_driver(cd->dev.driver);
	return d->request_tthe_debugfs_dir(ttsp);
}
