...
int cyttsp5_read_sensor_data(char const *name, char const *core_id,
			 int protect, u16 read_offset, u16 read_count,
			 u8 data_id, u8 *response, u8 *config,
			 u16 *actual_read_len, u8 *read_buf)
{
	...
	d->cmd->suspend_scanning(ttsp, protect);
	d->cmd->exec_panel_scan(ttsp, protect);
	...
	while(elem_count > 0) {
		...
		if (read_len <= 0)
			break;
		elem_size = *config & CY_CMD_RET_PANEL_ELMNT_SZ_MASK;
		elem_offset += read_len;
		elem_count -= read_len;
		rbuf += read_len*elem_size;
	}

 exit:
	d->cmd->resume_scanning(ttsp, protect);
	return rc;
}
EXPORT_SYMBOL_GPL(cyttsp5_read_sensor_data);

int cyttsp5_suspend_scanning(char const *name, char const *core_id,
			     int protect)
{
	struct cyttsp5_device *ttsp;
	struct cyttsp5_core *cd;
	...
	cd = ttsp->core;
	d = to_cyttsp5_core_driver(cd->dev.driver);
	rc = d->cmd->suspend_scanning(ttsp, protect);
	return 0;
}
EXPORT_SYMBOL_GPL(cyttsp5_suspend_scanning);

int cyttsp5_resume_scanning(char const *name, char const *core_id,
			    int protect)
{
	struct cyttsp5_device *ttsp;
	struct cyttsp5_core *cd;
	...
	cd = ttsp->core;
	d = to_cyttsp5_core_driver(cd->dev.driver);
	rc = d->cmd->resume_scanning(ttsp, protect);
	return rc;
}
