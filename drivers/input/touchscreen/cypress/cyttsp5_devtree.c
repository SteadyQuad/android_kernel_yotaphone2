...
static struct cyttsp5_core_platform_data *create_and_get_core_pdata(
		struct device_node *core_node, const char *id)
{
	...
	if(strcmp(id,"main_ttsp_core_front") == 0){
		pdata->loader_pdata = &_cyttsp5_loader_platform_data_front;
	} else {
		pdata->loader_pdata = &_cyttsp5_loader_platform_data_back;
	}
		...
		const char *adap_id, const char **core_id)
{
	...
	info.platform_data = create_and_get_core_pdata(core_node, info.id);
	if (IS_ERR(info.platform_data)) {
		rc = PTR_ERR(info.platform_data);
		goto fail;
	}
