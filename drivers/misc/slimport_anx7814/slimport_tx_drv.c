...
static int g_slimport_chipid = -1;
...
EXPORT_SYMBOL(hardware_power_ctl);
...
int get_slimport_chipid(void)
{
	return g_slimport_chipid;
}
...
void slimport_waitting_cable_plug_process(struct anx7816_data *td)
{
	int status=0;
	if(is_cable_detected()){
		if (td->hdmi_sp_ops->set_upstream_hpd) {
			status = td->hdmi_sp_ops->set_upstream_hpd(
					td->hdmi_pdev, 1);
			if (status)
...
void slimport_state_process (struct anx7816_data *td) 
{
	...
	case STATE_INIT:
		goto_next_system_state();
	case STATE_WAITTING_CABLE_PLUG:
		slimport_waitting_cable_plug_process(td);
...
void slimport_main_process(struct anx7816_data *td)
{
	slimport_state_process(td);
	slimport_int_rec();	
